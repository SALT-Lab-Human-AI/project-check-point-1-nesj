...
from utils.medication_safety import (
    apply_medication_safety_guardrails,
    get_medication_safety_system_prompt_addition
)
...
class CompanionAgent:
    ...
    def _get_system_prompt(self) -> str:
        ...
        medication_safety_rules = get_medication_safety_system_prompt_addition()
...
Generate greeting:"""
    ...
    def generate_response(
        ...
        try:
            ...
            # MEDICATION SAFETY GUARDRAILS - PRE-PROCESSING
            # Check if user is asking for medication advice (recommendations, drug names, dosages)
            # This must be handled with a safe response, not by the LLM
            safety_check = apply_medication_safety_guardrails(user_message)
            
            if safety_check["should_use_safe_response"] and safety_check["is_medication_query"]:
                # User is asking for medication advice - use safe response
                safe_response = safety_check["safe_response"]
                
                # Run PII/PHI sanitization on safe response
                user_msg_redacted, safe_response_redacted, contains_pii, pii_warning = sanitize_before_storage(
                    user_message, safe_response
                )
                
                # Save conversation with properly sanitized versions
                conversation = ConversationCRUD.save_conversation(
                    user_id=user_id,
                    message=user_msg_redacted,
                    response=safe_response_redacted,
                    sentiment_score=-0.3,
                    sentiment_label="concern",
                    conversation_type="medication_advice_declined"
                )
                
                # Add to vector store with safe response only
                self.memory_manager.add_conversation(
                    user_id=user_id,
                    conversation_id=conversation.id,
                    user_message=user_msg_redacted,
                    assistant_response=safe_response_redacted,
                    timestamp=conversation.timestamp
                )
                
                # Determine if we should alert caregiver (for emergency symptoms)
                # Use existing caregiver alert mechanism for consistency
                alert_sent = False
                ...
                if safety_check["is_emergency"]:
                    ...
                    emergency_concerns = safety_check["details"].get("matched_emergency_symptoms", [])
                    should_alert = True
                    
                    # Use existing alert mechanism
                    self.alert_caregiver_tool(
                        user_id=user_id,
                        alert_type="health_emergency",
                        description=f"User reported emergency symptoms: {', '.join(emergency_concerns)}. Message: '{user_message}'",
                        severity="critical"
                    )
                    alert_sent = True
                ...
                return {
                    ...
                    "medication_safety_triggered": True
                }
...
Respond naturally and warmly based on ALL the context provided."""
            ...
            # MEDICATION SAFETY GUARDRAILS - POST-PROCESSING
            # Filter AI response to catch any medication names/dosages that slipped through
            post_safety_check = apply_medication_safety_guardrails(user_message, ai_response)
            medication_was_filtered = False
            if post_safety_check["original_response_filtered"]:
                # AI gave medication advice despite instructions - use safe response
                print(f"MEDICATION SAFETY: Filtered response. Found: {post_safety_check['details']}")
                ai_response = post_safety_check["filtered_response"]
                medication_was_filtered = True
            ...
            # NOTE: This uses the potentially filtered ai_response (safe response), ensuring
            # no unsafe medication advice is stored in history or vector store
